var multiplayer = {
  ...
  // Initializing and Starting the Multiplayer Game
  currentLevel: 0,
  ...
  sendCommand: function(uids, details) {
    multiplayer.sendCommandForTick = true
  ...
  tickLoop: function() {
    ...
    if (multiplayer.currentTick <= multiplayer.lastReceivedTick) {
      ...
      if (!multiplayer.sendCommandForTick) {
        multiplayer.sendCommand()
      }
      multiplayer.currentTick++
      multiplayer.sendCommandForTick = false
    }
  },
...
$(window).keydown(function(e) {
  ...
  if (keyPressed === 13) {
    ...
    // Escape key pressed
    // Pressing escape key hides the chat box
    $('#chatmessage').hide()
    $('#chatmessage').val('')
    e.preventDefault()
  }
