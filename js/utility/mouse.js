var mouse = {
  ...
  buttonPressed: false,
  // whether or not the player is dragging and selecting with left mouse pressed
  dragSelect: false,
  // whether or not the mouse is inside the canvas
  insideCanvas: false,

  click: function(evt, rightClick) {
    // Player click inside the canvas
    ...
    if (!rightClick) { // Player left clicked
      ...
      if (clickedItem) { // Player right-clicked on something
        if (clickedItem.type !== 'terrain') {
          ...
          // Identify the first selected harvester from player's team (since only one can deploy at a time)
          for (var i = game.selectedItems.length - 1; i >= 0; i--) {
            var item = game.selectedItems[i]
        ...
        // Identity selected items from player's team that can move
        for (var i = game.selectedItems.length - 1; i >= 0; i--) {
          var item = game.selectedItems[i]
          if (item.team === game.team && (item.type === 'vehicles' || item.type === 'aircraft')) {
            uids.push(item.uid)
  ...
  itemUnderMouse: function() {
    ...
    for (var i = game.items.length - 1; i >= 0; i--) {
      ...
      if (item.type === 'buildings' || item.type === 'terrain') {
        if (item.lifeCode !== 'dead' && item.x <= (mouse.gameX)/game.gridSize && item.x >= (mouse.gameX - item.baseWidth)/game.gridSize && item.y <= mouse.gameY/game.gridSize && item.y >= (mouse.gameY - item.baseHeight)/game.gridSize) {
          return item
        }
      } else if (item.type === 'aircraft') {
        if (item.lifeCode !== 'dead' && Math.pow(item.x - mouse.gameX/game.gridSize, 2) + Math.pow(item.y - (mouse.gameY + item.pixelShadowHeight)/game.gridSize, 2) < Math.pow((item.radius)/game.gridSize, 2)) {
          return item
        }
      } else {
  ...
  init: function() {
    ...
    $mouseCanvas.mousemove(function(evt) {
      var offset = $mouseCanvas.offset()
      mouse.x = evt.pageX - offset.left
      mouse.y = evt.pageY - offset.top
    ...
    $mouseCanvas.click(function(evt) {
      mouse.click(evt, false)
      mouse.dragSelect = false
      return false
    })

    $mouseCanvas.mousedown(function(evt) {
      if (evt.which === 1) {
        // dragX and dragY are start point of dragging
    ...
    $mouseCanvas.bind('contextmenu', function(evt) {
      mouse.click(evt, true)
      return false
    })

    $mouseCanvas.mouseup(function(evt) {
      var shiftPressed = evt.shiftKey
      if (evt.which === 1) {
        ...
        if (mouse.dragSelect) {
          ...
          for (var i = game.items.length - 1; i >=0; i--) {
            ...
            if (item.type !== 'buildings' && item.selectable && item.team === game.team && x1 <= item.x && x2 >= item.x) {
              if ((item.type === 'vehicles' && y1 <= item.y && y2 >= item.y) || (item.type === 'aircraft' && (y1 <= item.y - item.pixelShadowHeight/game.gridSize) && (y2 >= item.y - item.pixelShadowHeight/game.gridSize))) {
    ...
    $mouseCanvas.mouseleave(function(evt) {
      mouse.insideCanvas = false
    })

    $mouseCanvas.mouseenter(function(evt) {
      mouse.buttonPressed = false
